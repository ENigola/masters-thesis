classdef slidingWindowDecoder < handle
    ...
    methods
        ...
        function createWindows(obj)
            obj.windows = zeros(obj.L, (obj.m+1)*obj.r);
            windowBlocks = zeros(1, obj.m+1);
            for i = 1:obj.L
                windowMainStart = mod((i-1)*(obj.m-1), obj.L*(obj.m-1)) + 1;
                windowMainEnd = mod(windowMainStart + obj.m - 1, obj.L*(obj.m-1));

                if windowMainStart < windowMainEnd
                    windowBlocks(1:obj.m) = windowMainStart:windowMainEnd;
                else
                    windowBlocks(1:obj.m) = [1:windowMainEnd windowMainStart:obj.L*(obj.m-1)];
                end

                windowBlocks(obj.m + 1) = obj.L * (obj.m - 1) + i;
        ...
        function createHWindows(obj, H)
            ...
            obj.CWindows = zeros(columnWeight, (obj.m+1)*obj.r, obj.L);
            for i = 1:obj.L
